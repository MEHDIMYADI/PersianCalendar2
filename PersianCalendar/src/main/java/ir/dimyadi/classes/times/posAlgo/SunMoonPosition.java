...
public class SunMoonPosition {
    ...
    public SunMoonPosition(double jd, double latitude, double longitude, double altitude, double ΔT) {
        ...
        Ecliptic moonPosEc = lunar.calculateMoonEclipticCoordinates(jd, ΔT);
        Ecliptic solarPosEc = solar.calculateSunEclipticCoordinatesAstronomic(jd - tau_Sun, ΔT);


        double E = Math.toRadians(solarPosEc.λ - moonPosEc.λ);

        Equatorial moonPosEq = lunar.calculateMoonEqutarialCoordinates(moonPosEc, jd, ΔT);
        Equatorial solarPosEq = solar.calculateSunEquatorialCoordinates(solarPosEc, jd, ΔT);
