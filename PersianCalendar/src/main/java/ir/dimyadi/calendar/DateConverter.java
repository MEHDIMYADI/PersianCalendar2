...
public final class DateConverter {
    ...
    public static IslamicDate civilToIslamic(CivilDate civil, int offset) {
        ...
        int b;
        //if ((lYear==2018) && (lMonth>=3)) {
        //    //rafe moshekl 3 rajab va shorou az 4 rajab dar sal 01/01/1397
        //    if ((lMonth==3)&&(lDay<21)) {
        //        b =1;
        //    } else
        //        b =1;
        //} else{
            b =1;
        //}
        return jdnToIslamic(civilToJdn(civil) + b + offset);
    }

    // b is for fix 1 day backward in ConverterFragment
    public static IslamicDate persianToIslamic(PersianDate persian) {
        long b = 1;
        return jdnToIslamic(persianToJdn(persian) + b);
    }
    ...
    private static long islamicToJdn(IslamicDate islamic) {
        ...
        int tableResult = IslamicDateConverter.hijriToJd(year, month, day);
        if (tableResult != -1)
            return tableResult;
    ...
    private static IslamicDate jdnToIslamic(long jd) {
        int[] tableResult = IslamicDateConverter.jdToHirji((int)jd);
        if (tableResult != null) {
