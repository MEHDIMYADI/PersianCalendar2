...
import ir.dimyadi.persiancalendar.Widget1x1;
import ir.dimyadi.persiancalendar.Widget2x2;
import ir.dimyadi.persiancalendar.Widget4x1;
import ir.dimyadi.persiancalendar.service.ApplicationService;
import ir.dimyadi.persiancalendar.view.activity.SplashActivity;
...
public class UpdateUtils {
    ...
    public void update(boolean updateDate) {
        ...
        if (applicationService != null && utils.isNotifyDate()) {
            ...
            {
                //Notification for SDK < Android Oreo
            applicationService.startForeground(
                ...
                CharSequence notificationTitle= context.getString(R.string.notification_calendar);
                String desc= context.getString(R.string.notification_calendar_desc);
                int imp= NotificationManager.IMPORTANCE_LOW;
                final String PRIMARY_CHANNEL="ir.dimyadi.persiancalendar";

                NotificationChannel mChannel = new NotificationChannel(PRIMARY_CHANNEL, notificationTitle,
                        imp);
                ...
                //Create Notification
                Notification n = new Notification.Builder(context,PRIMARY_CHANNEL)
                                .setOngoing(true)
                                .setContentTitle(utils.shape(title))
                                .setContentText(utils.shape(body))
                                ...
                                .setSmallIcon(icon)
                                .build();
                //mNotify.notify(NOTIFICATION_ID, n);
                applicationService.startForeground(NOTIFICATION_ID, n);
            }
        }
