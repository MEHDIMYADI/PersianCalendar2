package ir.dimyadi.persiancalendar.util;
...
import ir.dimyadi.shaper.arabicreshaper.ArabicShaping;
import ir.dimyadi.persiancalendar.R;
import ir.dimyadi.persiancalendar.adapter.ShapedArrayAdapter;
import ir.dimyadi.persiancalendar.entity.CityEntity;
import ir.dimyadi.persiancalendar.entity.DayEntity;
import ir.dimyadi.persiancalendar.entity.EventEntity;
import ir.dimyadi.persiancalendar.enums.CalendarTypeEnum;
import ir.dimyadi.persiancalendar.enums.SeasonEnum;
import ir.dimyadi.persiancalendar.service.BroadcastReceivers;
import ir.dimyadi.praytime.praytimes.CalculationMethod;
...
import ir.dimyadi.praytime.twaddington.TypefaceSpan;
...
import ir.dimyadi.calendar.AbstractDate;
import ir.dimyadi.calendar.CivilDate;
import ir.dimyadi.calendar.DateConverter;
import ir.dimyadi.calendar.DayOutOfRangeException;
import ir.dimyadi.calendar.IslamicDate;
import ir.dimyadi.calendar.PersianDate;

import static ir.dimyadi.persiancalendar.Constants.AM_IN_PERSIAN;
import static ir.dimyadi.persiancalendar.Constants.ARABIC_DIGITS;
import static ir.dimyadi.persiancalendar.Constants.BROADCAST_ALARM;
import static ir.dimyadi.persiancalendar.Constants.BROADCAST_RESTART_APP;
import static ir.dimyadi.persiancalendar.Constants.DARK_THEME;
import static ir.dimyadi.persiancalendar.Constants.DAYS_ICONS;
import static ir.dimyadi.persiancalendar.Constants.DAYS_ICONS_AR;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_ALTITUDE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_APP_LANGUAGE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_ATHAN_VOLUME;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_CITY;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_IRAN_TIME;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_ISLAMIC_OFFSET;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_LATITUDE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_LONGITUDE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_NOTIFY_DATE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_TESTAMENT_CODE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_UP_DATE;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_TESTAMENT;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_PERSIAN_DIGITS;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_PRAY_TIME_METHOD;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_SELECTED_WIDGET_TEXT_COLOR;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_WIDGET_CLOCK;
import static ir.dimyadi.persiancalendar.Constants.DEFAULT_WIDGET_IN_24;
import static ir.dimyadi.persiancalendar.Constants.KEY_EXTRA_PRAYER_KEY;
import static ir.dimyadi.persiancalendar.Constants.LIGHT_THEME;
import static ir.dimyadi.persiancalendar.Constants.PERSIAN_COMMA;
import static ir.dimyadi.persiancalendar.Constants.PREF_TESTAMENT_CODE;
import static ir.dimyadi.persiancalendar.Constants.PREF_UP_DATE;
import static ir.dimyadi.persiancalendar.Constants.PREF_TESTAMENT;
import static ir.dimyadi.persiancalendar.Constants.PERSIAN_DIGITS;
import static ir.dimyadi.persiancalendar.Constants.PM_IN_PERSIAN;
import static ir.dimyadi.persiancalendar.Constants.PREF_ALTITUDE;
import static ir.dimyadi.persiancalendar.Constants.PREF_APP_LANGUAGE;
import static ir.dimyadi.persiancalendar.Constants.PREF_ATHAN_ALARM;
import static ir.dimyadi.persiancalendar.Constants.PREF_ATHAN_GAP;
import static ir.dimyadi.persiancalendar.Constants.PREF_ATHAN_VOLUME;
import static ir.dimyadi.persiancalendar.Constants.PREF_GEOCODED_CITYNAME;
import static ir.dimyadi.persiancalendar.Constants.PREF_IRAN_TIME;
import static ir.dimyadi.persiancalendar.Constants.PREF_ISLAMIC_OFFSET;
import static ir.dimyadi.persiancalendar.Constants.PREF_LATITUDE;
import static ir.dimyadi.persiancalendar.Constants.PREF_LONGITUDE;
import static ir.dimyadi.persiancalendar.Constants.PREF_NOTIFY_DATE;
import static ir.dimyadi.persiancalendar.Constants.PREF_PERSIAN_DIGITS;
import static ir.dimyadi.persiancalendar.Constants.PREF_PRAY_TIME_METHOD;
import static ir.dimyadi.persiancalendar.Constants.PREF_SELECTED_LOCATION;
import static ir.dimyadi.persiancalendar.Constants.PREF_SELECTED_WIDGET_TEXT_COLOR;
import static ir.dimyadi.persiancalendar.Constants.PREF_THEME;
import static ir.dimyadi.persiancalendar.Constants.PREF_UP_DATE;
import static ir.dimyadi.persiancalendar.Constants.PREF_WIDGET_CLOCK;
import static ir.dimyadi.persiancalendar.Constants.PREF_WIDGET_IN_24;
...
public class Utils {
    ...
    public boolean isUpDate() {
        return prefs.getBoolean(PREF_UP_DATE, DEFAULT_UP_DATE);
    }

    public boolean isTestament() {
        return prefs.getBoolean(PREF_TESTAMENT, DEFAULT_TESTAMENT);
    }

    public String isTestamentCode() {
        return prefs.getString(PREF_TESTAMENT_CODE, DEFAULT_TESTAMENT_CODE);
    }
    ...
    public String getZekrDayName(AbstractDate date) {
        if (date instanceof IslamicDate)
            date = DateConverter.islamicToCivil((IslamicDate) date);
        else if (date instanceof PersianDate)
            date = DateConverter.persianToCivil((PersianDate) date);
        ...
        return zekrDays[date.getDayOfWeek() % 7];
    }
    ...
    private void setAlarm(PrayTime prayTime, long timeInMillis, int id) {
        ...
        if (!triggerTime.before(Calendar.getInstance())) {
            ...
            //Cleaned it and update for work in last API 23 or Above
            ...
            //New Code For AlarmManager for New API 23 or Above
    ...
    //Cleaned it and update for work in last API 23
    ...
    public Uri getAthanUri() {
        String defaultSoundUri = "android.resource://" + context.getPackageName() + "/" + R.raw.moazenzadeh;
    ...
    public void loadLanguageResource() {
        ...
        try {
            ...
            JSONArray zekrDaysArray = messages.getJSONArray("ZekrDays");
            for (int i = 0; i < 7; ++i)
                zekrDays[i] = zekrDaysArray.getString(i);
